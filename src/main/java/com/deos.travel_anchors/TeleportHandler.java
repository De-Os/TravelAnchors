...
public class TeleportHandler {
    ...
    public static Pair<BlockPos, String> getAnchorToTeleport(Level level, Player player, @Nullable BlockPos except) {
        if (!player.isShiftKeyDown()) {
            ...
            Optional<Pair<BlockPos, String>> anchor = TravelAnchorList.get(level).getAnchorsAround(player.position(), Math.pow(maxDistance, 2))
                    ...
                    .filter(p -> Math.abs(getAngleRadians(positionVec, p.getLeft(), player.getYRot(), player.getXRot())) <= Math.toRadians(Config.max_angle))
                    .min((p1, p2) -> {
    ...
    public static boolean shortTeleport(Level level, Player player, InteractionHand hand) {
        ...
        for (double i = Config.max_short_tp_distance; i >= 2; i -= 0.5) {
    ...
    public static double getMaxDistance(Player player) {
        var teleportationEnchantment = player.registryAccess().holderOrThrow(ModEnchantments.TELEPORTATION_RANGE);

        int mainHandLevel = player.getItemInHand(InteractionHand.MAIN_HAND).getEnchantmentLevel(teleportationEnchantment);
        int offHandLevel = player.getItemInHand(InteractionHand.OFF_HAND).getEnchantmentLevel(teleportationEnchantment);
        int lvl = Math.max(mainHandLevel, offHandLevel);
        return Config.max_distance * (1 + (lvl / 2d));
    }
